...
import DashMobileNavbar from "../navbar/navbarDashboard/DashMobileNavbar";
import useScreenSize from "@/app/hooks/useScreenSize";
import Grid from "./Grid";
import Sidebar from "../dasbboardSidebar/Sidebar";

function DashboardMainComponent() {
  const isMobile = useScreenSize(640);
  return isMobile ? (
    <div className="grid grid-cols-1 min-h-screen p-2 gap-4">
      <DashMobileNavbar />
      <Grid />
    </div>
  ) : (
